...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (!userSnap.exists() || !userSnap.data().seat_number) { alert("⚠️ 請先完成身分綁定！"); router.push("/login"); return; }
  ...
  const sortedSolutions = solutions.filter(s => selectedSubject === "全部" || s.subject === selectedSubject).sort((a, b) => a.subject.localeCompare(b.subject, 'zh-TW'));
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950/80 p-4 md:p-8 pb-20 text-slate-800 dark:text-slate-100 transition-colors duration-500">
      ...
      {!isVerified ? (
        ...
        <div className="max-w-5xl mx-auto flex flex-col gap-6">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-2"><select value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} className="appearance-none bg-white/70 dark:bg-slate-900/60 border dark:border-slate-700/50 rounded-full px-8 py-3.5 font-bold shadow-lg text-sm transition-colors cursor-pointer"><option value="全部">🔍 全部科目</option>{Array.from(new Set(solutions.map(s => s.subject))).map(sub => <option key={sub} value={sub}>{sub}</option>)}</select></motion.div>
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {sortedSolutions.map(sol => (
              <motion.div key={sol.id} variants={itemVariants} whileHover={{ y: -5, scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleViewSolution(sol)} className="group bg-white/60 dark:bg-slate-900/50 backdrop-blur-md p-6 md:p-8 rounded-[2.5rem] md:rounded-[3rem] border border-white dark:border-slate-700/50 shadow-lg cursor-pointer relative overflow-hidden transition-all">
        ...
        {viewingPreviewUrl && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-6 overflow-hidden">
            ...
            <motion.div initial={{ y: "100%", opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: "100%", opacity: 0 }} className="bg-white dark:bg-slate-900 rounded-t-[3rem] md:rounded-[3.5rem] w-full max-w-5xl h-[95vh] md:h-[85vh] flex flex-col overflow-hidden shadow-2xl relative z-10 border dark:border-slate-700/50">
              <div className="p-5 md:p-8 flex justify-between items-center border-b bg-white/80 dark:bg-slate-900/80 sticky top-0 z-20 transition-colors"><div className="flex items-center gap-3"><div className="p-2 bg-teal-50 dark:bg-teal-500/10 rounded-xl"><BookOpen size={20} className="text-teal-600" /></div><span className="font-black text-base md:text-lg">正在查閱解答</span></div><button onClick={() => setViewingPreviewUrl(null)} className="w-10 h-10 flex items-center justify-center bg-slate-100 dark:bg-slate-800 hover:bg-red-500 hover:text-white rounded-full font-bold transition-all">✕</button></div>
