...
const MaintenanceScreen = () => (
  <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-8 transition-colors duration-500">
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-3xl p-12 rounded-[3.5rem] shadow-2xl border border-white dark:border-slate-800 text-center max-w-md w-full relative z-10">
      ...
      <p className="text-slate-500 dark:text-slate-400 font-bold mb-10 text-sm">TerryEdu 目前正在進行升級。請等候開放。</p>
      <div className="py-2.5 px-8 bg-slate-100 dark:bg-slate-800 rounded-full inline-flex items-center gap-2 border border-slate-200 dark:border-slate-700 transition-colors"><ShieldCheck size={14} className="text-orange-500" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest tracking-[0.3em]">Maintenance</span></div>
    </motion.div>
  </div>
);
      ...
      {!isVerified ? (
        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-8">
          <div className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border border-white dark:border-slate-700/50 rounded-[2.5rem] p-6 md:p-8 flex justify-between items-center shadow-xl transition-colors">
            <div className="flex items-center gap-4"><div className="w-12 h-12 bg-teal-500 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-teal-500/20"><BookOpen size={24} /></div><h1 className="text-xl font-black italic">TerryEdu</h1></div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3 bg-slate-100/50 dark:bg-slate-800/50 p-1.5 pr-5 rounded-full border border-white dark:border-slate-700 transition-all"><img src={auth.currentUser?.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${userData?.name}`} className="w-9 h-9 rounded-full border-2 border-white shadow-sm" /><span className="font-black text-sm">{userData?.seat_number} 號 {userData?.name}</span><button onClick={() => { signOut(auth); router.push("/login"); }} className="ml-2 text-slate-400 hover:text-red-500 transition-colors"><LogOut size={18} /></button></div>
            </div>
          </div>
          <div className="px-2"><select value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} className="appearance-none bg-white/70 dark:bg-slate-900/60 backdrop-blur-md border border-white dark:border-slate-700/50 rounded-full px-10 py-4 font-black shadow-lg text-sm dark:text-slate-200 min-w-[180px] hover:bg-white transition-all outline-none cursor-pointer"><option value="全部">🔍 全部科目</option>{Array.from(new Set(solutions.map(s => s.subject))).map(sub => <option key={sub} value={sub}>{sub}</option>)}</select></div>
          <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">{solutions.filter(s => selectedSubject === "全部" || s.subject === selectedSubject).map(sol => (
