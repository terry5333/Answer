...
const MaintenanceScreen = () => (
  <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-8 transition-colors duration-500">
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-3xl p-12 rounded-[3.5rem] shadow-2xl border border-white dark:border-slate-800 text-center max-w-md w-full relative z-10">
      <div className="w-20 h-20 bg-orange-500 text-white rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-xl shadow-orange-500/20"><AlertTriangle size={40} /></div>
      <h1 className="text-2xl font-black mb-4 text-slate-800 dark:text-slate-100 italic">系統維護中</h1>
      <p className="text-slate-500 dark:text-slate-400 font-bold mb-10 text-sm leading-relaxed text-sm">正在進行功能優化。請等候老師通知開放。</p>
      <div className="py-2.5 px-8 bg-slate-100 dark:bg-slate-800 rounded-full inline-flex items-center gap-2 border border-slate-200 dark:border-slate-700 transition-colors"><ShieldCheck size={14} className="text-orange-500" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Maintenance</span></div>
    </motion.div>
  </div>
);
...
export default function DashboardPage() {
  ...
  if (isBlocked) return <MaintenanceScreen />;

  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950/80 p-4 md:p-8 pb-20 relative overflow-hidden transition-colors duration-500">
      ...
      {!isVerified ? (
        <div className="min-h-[80vh] flex items-center justify-center"><div className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-2xl p-12 rounded-[3.5rem] text-center shadow-2xl border border-white/50 dark:border-slate-800 max-w-sm w-full transition-colors"><h1 className="text-xl font-black mb-8 text-indigo-900 dark:text-indigo-300 flex items-center justify-center gap-2 italic"><ShieldCheck /> 安全驗證</h1><Turnstile siteKey={process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY!} onSuccess={() => setIsVerified(true)} /></div></div>
      ) : (
        <div className="max-w-6xl mx-auto flex flex-col gap-8">
          <div className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border border-white dark:border-slate-700/50 rounded-[2.5rem] p-6 md:p-8 flex justify-between items-center shadow-xl transition-colors">
      ...
      <AnimatePresence>
        {viewingPreviewUrl && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-6 overflow-hidden">
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} transition={{ type: "spring", stiffness: 250, damping: 30 }} className="bg-white dark:bg-slate-900 rounded-t-[3.5rem] md:rounded-[4rem] w-full max-w-6xl h-[96vh] md:h-[90vh] flex flex-col overflow-hidden shadow-2xl relative z-10 border dark:border-slate-800">
              <div className="p-6 md:p-9 flex justify-between items-center border-b dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-0"><div className="flex items-center gap-3"><div className="p-3 bg-teal-50 dark:bg-teal-500/10 rounded-2xl"><BookOpen size={24} className="text-teal-600" /></div><span className="font-black text-lg italic">正在閱覽解答</span></div><button onClick={() => setViewingPreviewUrl(null)} className="w-12 h-12 flex items-center justify-center bg-slate-100 dark:bg-slate-800 hover:bg-red-500 hover:text-white rounded-full font-bold shadow-sm transition-all">✕</button></div>
