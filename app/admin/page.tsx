...
import { 
  ...
  Edit2, Eye, Link, Unlink, Sun, Moon, BookOpen, ShieldCheck, Search, Trash2, CheckCircle, Trophy
} from "lucide-react";
...
export default function AdminPage() {
  ...
  const handleSyncCheck = async () => {
    ...
    } catch (e) { alert("比對失敗"); } finally { setIsSyncing(false); }
  };
            ...
            <motion.div key={activeTab} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-6">
                    <div className="bg-white/70 dark:bg-slate-900/50 p-8 rounded-[3rem] shadow-xl border border-white dark:border-slate-700/50 transition-colors">
                      <div className="flex justify-between items-center mb-6"><h2 className="text-lg font-black flex items-center gap-2"><Upload size={20}/> 上傳解答</h2><button onClick={handleSyncCheck} className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-5 py-2 rounded-full text-xs font-black shadow-sm hover:bg-indigo-600 hover:text-white transition-all"><Search size={14}/> 解答比對</button></div>
              ...
              {activeTab === "students" && (
                <div className="flex flex-col gap-8">
                  ...
                  <div className="bg-white/70 dark:bg-slate-900/50 p-12 rounded-[3.5rem] shadow-xl border border-white dark:border-slate-700/50 transition-colors"><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{students.map(s => (
                    <div key={s.id} className={`bg-white/90 dark:bg-slate-800/90 p-8 rounded-[3rem] flex flex-col items-center shadow-lg border-2 relative transition-all ${maintenance.active && maintenance.testers.includes(Number(s.seat_number)) ? 'border-orange-400' : 'border-transparent'}`}><div className="relative mb-6">
                    ...
                    </div><div className="font-black text-xl mb-6 dark:text-white">{s.name}</div><div className="flex flex-col w-full gap-3 border-t dark:border-slate-700 pt-6 mt-auto"><button onClick={() => setSelectedStudent(s)} className="flex items-center justify-center gap-2 bg-indigo-50 text-indigo-600 text-xs font-black py-3.5 rounded-full hover:bg-indigo-600 hover:text-white transition-all shadow-sm"><Eye size={16}/> 觀看紀錄</button>{s.bound_uid ? <button onClick={() => { if(confirm("確定解除連動？")) writeBatch(db).update(doc(db,"students",s.id),{bound_uid:null,bound_email:null,photo_url:null}).delete(doc(db,"users",s.bound_uid)).commit().then(fetchAdminData); }} className="bg-red-50 text-red-500 text-xs font-black py-3.5 rounded-full hover:bg-red-500 hover:text-white transition-all shadow-sm">解除連動</button> : <button onClick={() => handleManualBind(s.id)} className="bg-teal-50 dark:bg-teal-500/10 text-teal-600 text-xs font-black py-3.5 rounded-full hover:bg-teal-500 hover:text-white border border-teal-100 transition-all">手動綁定</button>}</div></div>))}</div></div>
                </div>
              )}
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-[3.5rem] p-8 md:p-10 w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl relative z-10 border border-white/20 transition-colors">
              ...
              <div className="overflow-y-auto flex-1 space-y-3 pr-2 custom-scrollbar">
                {viewLogs.filter(l => Number(l.seat_number) === Number(selectedStudent.seat_number)).map(log => {
                  ...
                  return (
                    <div key={log.id} className="group bg-white/70 dark:bg-slate-800/50 p-5 rounded-[2rem] flex justify-between items-center border border-white/50 shadow-sm hover:bg-white transition-all">
