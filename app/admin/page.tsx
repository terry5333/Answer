...
import { 
  ...
  Edit2, Eye, Link, Unlink, Sun, Moon, BookOpen, ShieldCheck, Search, Trash2, CheckCircle
} from "lucide-react";
...
export default function AdminPage() {
  ...
  const handleSyncCheck = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        // 取得 Firestore 中所有解答的檔案 ID
        const dbFileIds = solutions.map(sol => {
          const match = sol.file_url?.match(/[-\w]{25,}/);
          return match ? match[0] : null;
        }).filter(id => id !== null);

        // 找出雲端有、但資料庫沒有的檔案 (孤兒檔案)
        const orphans = cloudFiles.filter((cf: any) => !dbFileIds.includes(cf.id));
        setOrphanedFiles(orphans);
      }
    } catch (e) {
      alert("比對失敗，請確認 GAS 是否已部署 LIST_FILES 功能");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleSyncDelete = async (fileUrl: string) => {
    if (!confirm("確定要從雲端永久刪除此孤兒檔案嗎？")) return;
    try {
  ...
  const handleFullDelete = async (sol: any) => {
    if (!confirm(`確定要徹底刪除「${sol.title}」嗎？\n這將同時移除雲端硬碟中的檔案！`)) return;
    try {
      if (sol.file_url) {
        await fetch('/api/upload', {
          method: "POST",
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: "DELETE", fileUrl: sol.file_url }),
        });
      }
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-6">
                    <div className="bg-white/70 dark:bg-slate-900/50 p-8 rounded-[3rem] shadow-xl border border-white dark:border-slate-700/50 transition-colors">
                      <div className="flex justify-between items-center mb-6">
                        <h2 className="text-lg font-black flex items-center gap-2"><Upload size={20}/> 上傳解答</h2>
                        {/* 🚀 比對按鈕：找回 Clean UI 的質感按鈕 */}
                        <button onClick={handleSyncCheck} className="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 px-5 py-2.5 rounded-full text-xs font-black shadow-sm transition-all hover:bg-indigo-600 hover:text-white">
                          <Search size={14}/> 解答比對
                        </button>
                      </div>
                      ...
                      <div className="space-y-3">{solutions.map(sol => (
                        <div key={sol.id} className="flex justify-between items-center bg-white/80 dark:bg-slate-800/80 px-8 py-5 rounded-[2.5rem] shadow-sm border border-white dark:border-slate-700/50 group hover:bg-white/95 transition-all">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={() => handleFullDelete(sol)} className="bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400 text-[10px] px-4 py-2 rounded-full opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500 hover:text-white dark:hover:bg-red-500 dark:hover:text-white">刪除</button>
                          </div>
                        </div>
                      ))}</div>
                    </div>
      ...
      <AnimatePresence>
        {showSyncModal && (
          <div className="fixed inset-0 z-[130] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md" onClick={() => setShowSyncModal(false)} />
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 w-full max-w-2xl shadow-2xl relative z-10 border border-white/20 transition-colors">
              <div className="flex justify-between items-center mb-6 border-b pb-4 dark:border-slate-800">
                <div className="flex items-center gap-3 text-indigo-600 dark:text-indigo-400">
                  <Search size={24} />
                  <h3 className="text-xl font-black italic tracking-tight">雲端孤兒檔案清理</h3>
                </div>
                <button onClick={() => setShowSyncModal(false)} className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-full font-black text-slate-500">✕</button>
              </div>
              ...
              {isSyncing ? (
                <div className="py-20 text-center flex flex-col items-center gap-4">
                  ...
                  <p className="text-slate-500 font-bold animate-pulse">正在同步掃描雲端檔案庫...</p>
                </div>
              ) : (
                <div className="space-y-4 max-h-[450px] overflow-y-auto custom-scrollbar pr-2">
                  {orphanedFiles.length === 0 ? (
                    <div className="py-20 text-center flex flex-col items-center gap-4 text-green-500">
                      <CheckCircle size={48} />
                      <p className="font-black">非常乾淨！雲端與資料庫已完全對齊。</p>
                    </div>
                  ) : (
                    <>
                      <div className="bg-orange-50 dark:bg-orange-500/10 p-4 rounded-2xl border border-orange-100 dark:border-orange-500/20 mb-6">
                        <p className="text-xs text-orange-600 dark:text-orange-400 font-bold flex items-center gap-2">
                          <AlertTriangle size={14}/> 發現 {orphanedFiles.length} 個在雲端但不在資料庫的檔案。
                        </p>
                      </div>
                      {orphanedFiles.map(file => (
                        <div key={file.id} className="flex justify-between items-center p-5 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] border border-white/50 dark:border-slate-700/50 transition-all hover:bg-white dark:hover:bg-slate-800 shadow-sm group">
                          <div className="flex flex-col gap-1 overflow-hidden">
                            <span className="font-black text-sm truncate dark:text-slate-200">{file.name}</span>
                            <span className="text-[9px] text-slate-400 font-mono truncate tracking-tighter">{file.id}</span>
                          </div>
                          <button onClick={() => handleSyncDelete(file.url)} className="bg-red-50 text-red-500 p-3 rounded-2xl opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500 hover:text-white shadow-sm">
                            <Trash2 size={16}/>
                          </button>
                        </div>
                      ))}
        ...
        {showTesterModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 w-full max-w-2xl shadow-2xl relative z-10 border border-white/20">
              <h3 className="text-xl font-black mb-4 flex items-center gap-2 text-orange-500 tracking-tight italic">< ShieldCheck /> 設定維護期測試人員</h3>
