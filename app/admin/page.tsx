...
export default function AdminPage() {"use client";
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        ...
        <div className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border border-white dark:border-slate-700/50 rounded-[2.5rem] p-6 px-10 flex justify-between items-center shadow-xl transition-colors">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { signOut(auth); router.push("/login"); }} className="bg-slate-200 dark:bg-slate-800 px-6 py-2.5 rounded-full font-bold text-sm shadow-sm hover:bg-slate-300 transition-colors">登出</button>
          </div>
        </div>
        ...
        {!isVerified ? (
          <div className="flex justify-center py-20"><div className="bg-white/80 dark:bg-slate-900/70 p-12 rounded-[3rem] shadow-2xl border border-white dark:border-slate-700/50 text-center"><h2 className="text-xl font-bold mb-8 text-indigo-900 dark:text-indigo-300 tracking-tight">管理員安全驗證</h2><Turnstile siteKey={process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY!} onSuccess={() => setIsVerified(true)} /></div></div>
        ) : (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-6">
                    <div className="bg-white/70 dark:bg-slate-900/50 p-8 rounded-[3rem] shadow-xl border border-white dark:border-slate-700/50 transition-colors">
                      <h2 className="text-lg font-black mb-6 flex items-center gap-2"><Upload size={20}/> 上傳解答檔案</h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{students.map(s => (
                      <div key={s.id} className={`bg-white/90 dark:bg-slate-800/90 p-8 rounded-[3rem] flex flex-col items-center shadow-lg border-2 relative transition-all ${maintenance.active && maintenance.testers.includes(s.seat_number) ? 'border-orange-400' : 'border-transparent'}`}>
                        ...
                        <div className="flex flex-col w-full gap-3 border-t dark:border-slate-700 pt-6 mt-auto">
                          <button onClick={() => setSelectedStudent(s)} className="flex items-center justify-center gap-2 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 text-xs font-black py-3.5 rounded-full hover:bg-indigo-600 hover:text-white transition-all shadow-sm"><Eye size={16}/> 查看觀看紀錄</button>
                          {s.bound_uid ? <button onClick={() => { if(confirm("解除？")) writeBatch(db).update(doc(db,"students",s.id),{bound_uid:null,bound_email:null,photo_url:null}).delete(doc(db,"users",s.bound_uid)).commit().then(fetchAdminData); }} className="bg-red-50 dark:bg-red-500/10 text-red-500 text-xs font-black py-3.5 rounded-full hover:bg-red-500 hover:text-white transition-all shadow-sm">解除連動</button> : <button onClick={() => handleManualBind(s.id)} className="bg-teal-50 dark:bg-teal-500/10 text-teal-600 dark:text-teal-400 text-xs font-black py-3.5 rounded-full hover:bg-teal-500 hover:text-white transition-all border border-teal-100">手動綁定</button>}
                        </div>
                      </div>
                    ))}</div>
                  </div>
              ...
              {activeTab === "reports" && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="bg-white/70 dark:bg-slate-900/50 p-10 rounded-[3.5rem] shadow-xl border border-white dark:border-slate-700/50 h-[450px] flex flex-col items-center transition-colors">
                    <div className="flex justify-between w-full mb-6"><h2 className="text-lg font-black flex items-center gap-2"><BarChart3 size={20}/> 熱度分析</h2><div className="flex gap-2"><button onClick={fetchAdminData} className="bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 px-4 py-2 rounded-full text-[10px] font-bold shadow-sm transition-all active:scale-95"><RefreshCw size={12}/> 刷新</button><button onClick={handleDataRepair} className="bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 px-4 py-2 rounded-full text-[10px] font-bold border border-red-100 shadow-sm hover:bg-red-100 transition-all active:scale-95"><AlertTriangle size={12}/> 強制校正</button></div></div>
                  ...
                  <div className="bg-white/70 dark:bg-slate-900/50 p-10 rounded-[3.5rem] shadow-xl border border-white dark:border-slate-700/50 overflow-y-auto max-h-[450px] custom-scrollbar transition-colors">
                    <h2 className="text-lg font-black mb-8 dark:text-slate-100">🔥 熱門排行</h2>
        ...
        {showTesterModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 w-full max-w-2xl shadow-2xl relative z-10 border border-white/20">
              <h3 className="text-xl font-black mb-4 flex items-center gap-2 text-orange-500 tracking-tight italic">< ShieldCheck /> 設定測試人員</h3>
      ...
      <AnimatePresence>
        {selectedStudent && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-[3.5rem] p-8 md:p-10 w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl relative z-10 border border-white/20 transition-colors">
        ...
        {viewingPreviewUrl && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-0 md:p-6 overflow-hidden">
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", stiffness: 250, damping: 30 }} className="bg-white dark:bg-slate-900 rounded-t-[3rem] md:rounded-[3.5rem] w-full max-w-5xl h-[95vh] flex flex-col relative z-10 overflow-hidden shadow-2xl border border-white/20">
              <div className="p-8 flex justify-between items-center border-b dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 sticky top-0 z-20"><div className="flex items-center gap-3"><div className="p-2 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl"><BookOpen size={20} className="text-indigo-600" /></div><span className="font-black text-lg italic">正在查閱解答 (無痕預覽)</span></div><button onClick={() => setViewingPreviewUrl(null)} className="w-10 h-10 bg-slate-100 dark:bg-slate-800 hover:bg-red-500 hover:text-white rounded-full font-bold shadow-sm transition-all">✕</button></div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="bg-white/70 dark:bg-slate-900/50 p-8 rounded-[3rem] shadow-xl border border-white dark:border-slate-700/50 h-fit transition-colors">
                    ...
                    <div className="flex gap-2 mb-6">
                      <input value={newSubject} onChange={e => setNewSubject(e.target.value)} placeholder="新科目..." className="flex-1 rounded-full px-5 py-3 bg-white dark:bg-slate-800 outline-none text-sm shadow-inner transition-colors" />
                      <button onClick={async () => { if(newSubject){ await addDoc(collection(db,"subjects"),{name:newSubject}); setNewSubject(""); fetchAdminData(); }}} className="bg-indigo-600 text-white w-12 h-12 rounded-full font-bold shadow-lg">+</button>
                    </div>
                    ...
                    <div className="bg-white/70 dark:bg-slate-900/50 p-8 rounded-[3rem] shadow-xl border border-white dark:border-slate-700/50 transition-colors">
                      ...
                      <div className="space-y-3">{solutions.map(sol => (
                        <div key={sol.id} className="flex justify-between items-center bg-white/80 dark:bg-slate-800/80 px-8 py-5 rounded-[2.5rem] shadow-sm border border-white dark:border-slate-700/50 group hover:bg-white/95 transition-all">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => setViewingPreviewUrl(sol.file_url ? sol.file_url.replace(/\/view.*/, "/preview") : `https://drive.google.com/file/d/${sol.drive_file_id}/preview`)} className="bg-teal-50 dark:bg-teal-500/10 text-teal-600 dark:text-teal-400 text-[10px] px-4 py-2 rounded-full opacity-0 group-hover:opacity-100 transition-all hover:bg-teal-500 hover:text-white dark:hover:bg-teal-500 dark:hover:text-white">預覽</button>
                            <button onClick={() => deleteDoc(doc(db,"solutions",sol.id)).then(fetchAdminData)} className="bg-red-50 dark:bg-red-500/10 text-red-500 text-[10px] px-4 py-2 rounded-full opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500 hover:text-white dark:hover:bg-red-500 dark:hover:text-white">刪除</button>
                          </div>
                        </div>
                      ))}</div>
                    </div>
        ...
        {viewingPreviewUrl && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-0 md:p-6 overflow-hidden">
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} className="bg-white dark:bg-slate-900 rounded-t-[3rem] md:rounded-[3.5rem] w-full max-w-5xl h-[95vh] flex flex-col relative z-10 overflow-hidden shadow-2xl">
              <div className="p-8 flex justify-between items-center border-b dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 sticky top-0 z-20"><div className="flex items-center gap-3"><div className="p-2 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl"><BookOpen size={20} className="text-indigo-600" /></div><span className="font-black text-lg">解答預覽 (無痕)</span></div><button onClick={() => setViewingPreviewUrl(null)} className="w-10 h-10 bg-slate-100 dark:bg-slate-800 hover:bg-red-500 hover:text-white rounded-full font-bold transition-all shadow-sm">✕</button></div>
