...
export default function AdminPage() {
  ...
  const fetchSystemSettings = async () => {
    try {
      ...
      if (vSnap.exists()) {
        setSystemVersion(vSnap.data().version || "v2.0.0");
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950/80 p-4 md:p-8 text-slate-800 dark:text-slate-100 transition-colors duration-500 pb-24">
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        ...
        {/* 導航切換 */}
        <div className="flex justify-center gap-2 bg-white/70 dark:bg-slate-900/60 p-2 rounded-full shadow-lg border border-white/50 dark:border-slate-700/50 sticky top-4 z-40 transition-colors overflow-x-auto no-scrollbar">
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-6">
                    <div className="bg-white/70 dark:bg-slate-900/50 p-8 rounded-[3rem] shadow-xl border border-white dark:border-slate-700/50 transition-colors">
                      ...
                      <div className="bg-indigo-50/50 dark:bg-indigo-900/20 p-4 rounded-2xl mb-6 text-xs text-indigo-700 dark:text-indigo-300 font-bold flex items-center gap-2 border border-indigo-100 dark:border-indigo-800/50">
                        ...
                        <p>請先將 PDF 拖曳至 Google Drive 資料夾，點擊右方 <strong className="bg-white/60 dark:bg-black/20 px-1.5 py-0.5 rounded">🔄 刷新</strong> 按鈕，即可選擇並綁定。此方式不受大小限制！</p>
                      </div>
                      <form onSubmit={handleBindSolution} className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 items-center w-full">
                          <div className="flex-1 flex w-full gap-2 items-center">
                            ...
                            <button type="button" onClick={fetchDriveFiles} className="bg-white dark:bg-slate-800 p-3.5 rounded-full shadow-md hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-slate-500 shrink-0 active:scale-95">
                              <RefreshCw size={18} className={isFetchingDrive ? "animate-spin text-indigo-500" : ""} />
                            </button>
                          </div>
                          <button disabled={isUploading || availableFiles.length === 0} className="w-full sm:w-auto bg-indigo-600 text-white font-black py-3.5 px-10 rounded-full shadow-lg disabled:opacity-50 text-sm transition-all active:scale-95 shrink-0">
                ...
                <div className="flex flex-col gap-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white/70 dark:bg-slate-900/50 p-6 rounded-[2.5rem] shadow-xl border border-white dark:border-slate-700/50 flex flex-col justify-center gap-4 transition-colors">
                      ...
                      <button onClick={() => { if(maintenance.active) setDoc(doc(db,"settings","maintenance"),{active:false,testers:[]}).then(() => fetchSystemSettings()); else setShowTesterModal(true); }} className={`w-full py-3 rounded-full font-black text-xs shadow-md transition-all ${maintenance.active ? 'bg-slate-200 text-slate-600 hover:bg-slate-300' : 'bg-orange-500 text-white hover:bg-orange-600'}`}>{maintenance.active ? '關閉維護開放登入' : '啟動維護模式'}</button>
                    </div>
                    ...
                    <div className="bg-white/70 dark:bg-slate-900/50 p-6 rounded-[2.5rem] shadow-xl border border-white dark:border-slate-700/50 flex flex-col justify-center transition-colors lg:col-span-3 xl:col-span-1">
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <textarea value={systemNotes} onChange={e => setSystemNotes(e.target.value)} placeholder="輸入更新內容，可換行..." rows={2} className="w-full bg-white dark:bg-slate-800 rounded-xl px-4 py-3 font-bold text-xs shadow-inner transition-colors outline-none resize-none custom-scrollbar" />
          ...
          <div className="fixed inset-0 z-[140] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 w-full max-w-sm shadow-2xl relative z-10 border border-white/20 transition-colors">
              ...
              <div className="mb-8">
                ...
                <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full px-6 py-4 font-black text-lg outline-none focus:border-teal-500 transition-colors dark:text-white" autoFocus />
              </div>
              <div className="flex gap-3">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-[3.5rem] p-8 md:p-10 w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl relative z-10 border border-white/20 transition-colors">
              <div className="flex justify-between items-center mb-6 pb-4 border-b dark:border-slate-800">
                ...
                <button onClick={() => setSelectedStudent(null)} className="h-10 w-10 bg-slate-100 dark:bg-slate-800 rounded-full font-black text-slate-500 hover:bg-slate-200 transition-colors flex items-center justify-center">✕</button>
              </div>
